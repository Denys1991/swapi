<template>
    <div class="wrapper">
        <div class="header">
            <div class="soc">
                <div class="soc_facebook">
                    <a href="" target="_blank" rel="noopener noreferrer" title="facebook">
                        <svg fill="#999" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px"><path d="M 12 2 C 6.4889971 2 2 6.4889971 2 12 C 2 17.511003 6.4889971 22 12 22 C 17.511003 22 22 17.511003 22 12 C 22 6.4889971 17.511003 2 12 2 z M 12 4 C 16.430123 4 20 7.5698774 20 12 C 20 16.014467 17.065322 19.313017 13.21875 19.898438 L 13.21875 14.384766 L 15.546875 14.384766 L 15.912109 12.019531 L 13.21875 12.019531 L 13.21875 10.726562 C 13.21875 9.7435625 13.538984 8.8710938 14.458984 8.8710938 L 15.935547 8.8710938 L 15.935547 6.8066406 C 15.675547 6.7716406 15.126844 6.6953125 14.089844 6.6953125 C 11.923844 6.6953125 10.654297 7.8393125 10.654297 10.445312 L 10.654297 12.019531 L 8.4277344 12.019531 L 8.4277344 14.384766 L 10.654297 14.384766 L 10.654297 19.878906 C 6.8702905 19.240845 4 15.970237 4 12 C 4 7.5698774 7.5698774 4 12 4 z"/></svg>
                    </a>
                </div>
                <div class="soc_instagram">
                    <a href="" target="_blank" rel="noopener noreferrer" title="instagram">
                        <svg fill="#999" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px">    <path d="M 8 3 C 5.243 3 3 5.243 3 8 L 3 16 C 3 18.757 5.243 21 8 21 L 16 21 C 18.757 21 21 18.757 21 16 L 21 8 C 21 5.243 18.757 3 16 3 L 8 3 z M 8 5 L 16 5 C 17.654 5 19 6.346 19 8 L 19 16 C 19 17.654 17.654 19 16 19 L 8 19 C 6.346 19 5 17.654 5 16 L 5 8 C 5 6.346 6.346 5 8 5 z M 17 6 A 1 1 0 0 0 16 7 A 1 1 0 0 0 17 8 A 1 1 0 0 0 18 7 A 1 1 0 0 0 17 6 z M 12 7 C 9.243 7 7 9.243 7 12 C 7 14.757 9.243 17 12 17 C 14.757 17 17 14.757 17 12 C 17 9.243 14.757 7 12 7 z M 12 9 C 13.654 9 15 10.346 15 12 C 15 13.654 13.654 15 12 15 C 10.346 15 9 13.654 9 12 C 9 10.346 10.346 9 12 9 z"/></svg>
                    </a>
                </div>
                <div class="soc_twitter">
                    <a href="" target="_blank" rel="noopener noreferrer" title="twitter">
                        <svg fill="#999" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px">    <path d="M22,3.999c-0.78,0.463-2.345,1.094-3.265,1.276c-0.027,0.007-0.049,0.016-0.075,0.023c-0.813-0.802-1.927-1.299-3.16-1.299 c-2.485,0-4.5,2.015-4.5,4.5c0,0.131-0.011,0.372,0,0.5c-3.353,0-5.905-1.756-7.735-4c-0.199,0.5-0.286,1.29-0.286,2.032 c0,1.401,1.095,2.777,2.8,3.63c-0.314,0.081-0.66,0.139-1.02,0.139c-0.581,0-1.196-0.153-1.759-0.617c0,0.017,0,0.033,0,0.051 c0,1.958,2.078,3.291,3.926,3.662c-0.375,0.221-1.131,0.243-1.5,0.243c-0.26,0-1.18-0.119-1.426-0.165 c0.514,1.605,2.368,2.507,4.135,2.539c-1.382,1.084-2.341,1.486-5.171,1.486H2C3.788,19.145,6.065,20,8.347,20 C15.777,20,20,14.337,20,8.999c0-0.086-0.002-0.266-0.005-0.447C19.995,8.534,20,8.517,20,8.499c0-0.027-0.008-0.053-0.008-0.08 c-0.003-0.136-0.006-0.263-0.009-0.329c0.79-0.57,1.475-1.281,2.017-2.091c-0.725,0.322-1.503,0.538-2.32,0.636 C20.514,6.135,21.699,4.943,22,3.999z M18,8.999c0,4.08-2.957,8.399-8.466,8.943C10.28,17.413,11,16.662,11,16.662 S8,14,7.775,10.522C8.81,10.838,9.888,10.999,11,10.999h2v-2.5c0-0.001,0-0.001,0-0.001C13.002,7.118,14.12,6,15.5,6 C16.881,6,18,7.119,18,8.5C18,8.5,18,8.92,18,8.999z"/></svg>
                    </a>
                </div>
                <div class="soc_youtube">
                    <a href="" target="_blank" rel="noopener noreferrer" title="youtube">
                        <svg fill="#999" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px">    <path d="M 12 4 C 12 4 5.7455469 3.9999687 4.1855469 4.4179688 C 3.3245469 4.6479688 2.6479687 5.3255469 2.4179688 6.1855469 C 1.9999687 7.7455469 2 12 2 12 C 2 12 1.9999687 16.254453 2.4179688 17.814453 C 2.6479687 18.675453 3.3255469 19.352031 4.1855469 19.582031 C 5.7455469 20.000031 12 20 12 20 C 12 20 18.254453 20.000031 19.814453 19.582031 C 20.674453 19.352031 21.352031 18.674453 21.582031 17.814453 C 22.000031 16.254453 22 12 22 12 C 22 12 22.000031 7.7455469 21.582031 6.1855469 C 21.352031 5.3255469 20.674453 4.6479688 19.814453 4.4179688 C 18.254453 3.9999687 12 4 12 4 z M 12 6 C 14.882 6 18.490875 6.1336094 19.296875 6.3496094 C 19.465875 6.3946094 19.604391 6.533125 19.650391 6.703125 C 19.891391 7.601125 20 10.342 20 12 C 20 13.658 19.891391 16.397875 19.650391 17.296875 C 19.605391 17.465875 19.466875 17.604391 19.296875 17.650391 C 18.491875 17.866391 14.882 18 12 18 C 9.119 18 5.510125 17.866391 4.703125 17.650391 C 4.534125 17.605391 4.3956094 17.466875 4.3496094 17.296875 C 4.1086094 16.398875 4 13.658 4 12 C 4 10.342 4.1086094 7.6011719 4.3496094 6.7011719 C 4.3946094 6.5331719 4.533125 6.3946094 4.703125 6.3496094 C 5.508125 6.1336094 9.118 6 12 6 z M 10 8.5351562 L 10 15.464844 L 16 12 L 10 8.5351562 z"/></svg>
                    </a>
                </div>
            </div>
            <div class="img_logo">
                <img src="../assets/img/sw_logo.png" alt="logo">
            </div>
            <div class="searchlogin">
                <div class="search">
                    <input type="text" placeholder="Search Star Wars">
                    <div>
                        <svg fill="#999" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 30 30" width="24px" height="24px"><path d="M 13 3 C 7.4889971 3 3 7.4889971 3 13 C 3 18.511003 7.4889971 23 13 23 C 15.396508 23 17.597385 22.148986 19.322266 20.736328 L 25.292969 26.707031 A 1.0001 1.0001 0 1 0 26.707031 25.292969 L 20.736328 19.322266 C 22.148986 17.597385 23 15.396508 23 13 C 23 7.4889971 18.511003 3 13 3 z M 13 5 C 17.430123 5 21 8.5698774 21 13 C 21 17.430123 17.430123 21 13 21 C 8.5698774 21 5 17.430123 5 13 C 5 8.5698774 8.5698774 5 13 5 z"/></svg>
                    </div>
                </div>
                <div class="login">
                    <p>LOG IN</p>
                    <div></div>
                    <p>SING UP</p>
                </div>
            </div>
            <div class="btn_sidebar" @click="changeBtnSidebar()">
                <div class="btn_sidebar-line "></div>
            </div>
        </div>  
        
    </div>
    <div class="wrapperNav">
        
        <nav class="nav">
            <div v-for="nav in navigation" :key="nav">{{nav}}</div>
        </nav>  
            
    </div>
</template>
<script>
    export default{
        data(){
            return{
                nav:'',
                navigation:['People', 'Planets', 'Films', 'Species', 'Vehicles', 'Starships'],
            };
        },
        methods:{
            changeBtnSidebar(){
                const btnSideBar = document.querySelector('.btn_sidebar');
                btnSideBar.classList.toggle('btn_active')
            }
        }
    }

</script>
<style lang="scss" scope>
//__________________________________________________________________header
.wrapper{
    
    max-width: 1200px;
    margin: 0 auto;
}
.header{
    display: flex;
    justify-content: space-between;
    padding: 10px;
}
.soc{
    padding-top: 10px;
    display: flex;
    gap: 15px;
}
.soc div a svg:hover{
    fill: white
}
.img_logo{
    transition: all .5s ease;
    margin: 10px;
}
.img_logo:hover{
    cursor: pointer;
    transform: scale(1.2);
}
.login{
    display: flex;
    align-items: center;
    justify-content: end;
}
.login div {
    border-left: 2px solid #999;
    width: 10px;
    height: 10px;
    border-right: 2px solid #999;
    transform: skewX(-16deg);
    margin: 0 10px;
}
.login p {
    color: #999;
    cursor: pointer;
}
.login p:hover{
    color: white;
}
.search{
    display: flex;
    align-items: center;
    justify-content: end;
    width: 100%;
    position: relative;
    padding-bottom: 15px;
}
.search div {
    position: absolute;
    top: 5px;
    right: 10px;
    
}
.search input{
    background-color: rgb(85, 85, 85);
    border: none;
    padding: 10px 40px 10px 10px;
    border-radius: 5px;
    color: white;
}
.search input:focus, .search input:active{
    border: none;
    outline: none;
    background-color: rgb(87, 85, 85);
}
//_________________________________________________________________nav
.wrapperNav{
    border-top: 1px solid rgb(95, 94, 94);
    border-bottom: 1px solid rgb(95, 94, 94);
    margin: 20px 0px 0px;
    display: flex;
    justify-content: center;
    width: 100%;
}

.nav{
    max-width: 1200px;
    width: 100%;
    display: flex;
    justify-content: space-between;    
    color: #999;
    /*border-top: 1px solid rgb(95, 94, 94);
    border-bottom: 1px solid rgb(95, 94, 94);*/
    flex-wrap: wrap;
    font-size: 25px;
    margin: 0 10px;
}
.nav div{
    padding: 10px 0;
    border-left: 1px solid rgb(95, 94, 94);
    cursor: pointer;
    width: 16.6%;
    text-align: center;
    /*border-bottom:  1px solid rgb(95, 94, 94);*/
}
.nav div:hover{
    color: white;
}
.nav:last-child{
    border-right: 1px solid rgb(95, 94, 94);
}
//____________________________________________________________________btn_sidebar
.btn_sidebar{
    display: none;
    width: 60px;
    height: 60px;
    border: 1px solid #999;
    position: absolute;
    right: 10px;
    top: 10px;
    background-color: #000;
}
.btn_sidebar-line{
    width: 40px;
    height: 3px;
    background-color: #999;
    position: absolute;
    top: 27.5px;
    left: 10px;
    &::before{
        content: ' ';
        width: 40px;
        height: 3px;
        background-color: #999;
        position: absolute;
        top: -10px;
        left: 0;
    }
    &::after{
        content: ' ';
        width: 40px;
        height: 3px;
        background-color: #999;
        position: absolute;
        bottom: -10px;
        left: 0;
    }
}
.btn_sidebar:hover{
    border: 1px solid #fff;
    .btn_sidebar-line{
        background-color: #fff; 
        &::before{        
            background-color: #fff;        
        }
        &::after{       
            background-color: #fff;        
        }
    }
}
.btn_active{
    .btn_sidebar-line{
        transition: all .5s ease;
        transform: rotate(45deg);
        &::before{        
            transform: rotate(-90deg);
            top:0;
        }
        &::after{       
            display:none;
        }
    }
}
//____________________________________________________________________media
@media (max-width: 650px){
    .nav div{
        width: 33.3%;
        padding: 30px 0;
        border-bottom:  1px solid rgb(95, 94, 94);
    }
}
@media (max-width: 600px) {
    .searchlogin{
        display: none;
    }
    .soc{
        display: none;
    }
    .header{
        justify-content: center;
    }
    .btn_sidebar{
        display: block;
        opacity: .5;
    }
}
@media(max-width: 450px){
    .header{
        justify-content: flex-start;
    }    
    .nav{
        font-size: 22px;
    }
}

</style>